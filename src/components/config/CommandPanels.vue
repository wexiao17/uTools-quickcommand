...
export default {
  ...
  data() {
    return {
      ...
      lastWheelTime: 0,
      previousCardStyle: null, // 添加变量跟踪上一次的卡片样式
    };
  },
  props: {
    footerBarHeight: {
  ...
  mounted() {
    ...
    // 记录初始卡片样式
    this.previousCardStyle = this.$root.profile.commandCardStyle;
  },
  beforeUnmount() {
  ...
  watch: {
    ...
    '$root.profile.commandCardStyle': {
      handler(newStyle, oldStyle) {
        this.previousCardStyle = oldStyle;
        // 在下一个渲染周期调整容器尺寸
        this.$nextTick(() => {
          this.adjustContainerSize();
          // 如果是从mini切换到其他样式，需要特殊处理
          if (oldStyle === 'mini' && newStyle !== 'mini') {
            // 强制重新计算布局
  ...
  computed: {
    ...
    currentTagQuickCommands() {
      ...
      const sortCommands = (cmds) => {
        ...
        const inactiveCmds = cmds.filter(
          (cmd) =>
            !this.activatedQuickCommandFeatureCodes.includes(cmd.features.code),
        );

        // 对每组命令进行排序：先按窗口类型，再按字母顺序
        const sortByTypeAndName = (cmdList) => {
          return cmdList.sort((a, b) => {
            // 1. 先按照命令类型排序（窗口类型的排在最后）
            const aIsWindow = a.features.cmds.some(
              (cmd) => typeof cmd === "object" && cmd.type === "window",
            );
            const bIsWindow = b.features.cmds.some(
              (cmd) => typeof cmd === "object" && cmd.type === "window",
            );

            if (aIsWindow !== bIsWindow) {
              return aIsWindow ? 1 : -1;
            }

            // 2. 再按照字母顺序排序
            return a.features.explain.localeCompare(b.features.explain, "zh-CN");
          });
        };

        // 先显示已激活的，再显示未激活的
        return [...sortByTypeAndName(activatedCmds), ...sortByTypeAndName(inactiveCmds)];
      };
      ...
      switch (this.currentTag) {
        ...
        case "刚关命令":
          ...
          return sortCommands(
            commands.filter((cmd) =>
              this.commandManager.state.recentlyDisabledCommands.includes(
                cmd.features.code,
              ),
            ),
          );
  ...
  methods: {
    ...
    adjustContainerSize() {
      ...
      if (this.$root.profile.commandCardStyle === "mini") {
        ...
        // 如果是从mini切换过来的，添加额外处理
        if (this.previousCardStyle === 'mini') {
          // 确保滚动区域重置
    ...
    handleScrollWheel(event) {
      ...
      const isAtBottom =
        scrollInfo.verticalPosition >=
        scrollInfo.verticalSize - scrollInfo.verticalContainerSize - 5; // 5px误差容忍
      const hasScrollbar =
        scrollInfo.verticalSize > scrollInfo.verticalContainerSize;
